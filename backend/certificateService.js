...
async function generateCertificate(userId, courseId) {
  ...
  try {
    ...
    // Generate QR code
    const qrSize = 120;
    const qrCodeBase64 = await QRCode.toDataURL(`http://localhost:5000/verify.html?code=${certNumber}`, {
      errorCorrectionLevel: 'H',
      margin: 1,
      width: qrSize
    });

    // 5. Generate JPG with QR code
    ...
    logger.info('Certificate record inserted successfully', { 
      id: certUuid,
